              ...
              <div className="hidden md:block overflow-x-auto">
                <table className="w-full text-left text-sm border-collapse">
                  <thead className="bg-slate-100 dark:bg-[#141414] text-slate-700 dark:text-[#888888] font-mono text-xs uppercase border-b border-slate-200 dark:border-[#222222]">
                    <tr>
                      <th className="py-3.5 px-3 xl:px-4 font-semibold whitespace-nowrap">Request ID</th>
                      <th className="py-3.5 px-3 xl:px-4 font-semibold whitespace-nowrap">Ambulance Unit</th>
                      <th className="py-3.5 px-3 xl:px-4 font-semibold whitespace-nowrap">Bed Category</th>
                      ...
                      <th className="py-3.5 px-3 xl:px-4 font-semibold whitespace-nowrap">Request Time</th>
                      <th className="py-3.5 px-3 xl:px-4 font-semibold text-center whitespace-nowrap">Status</th>
                      <th className="py-3.5 px-3 xl:px-4 font-semibold text-center whitespace-nowrap min-w-[190px] xl:min-w-[230px]">Decision Actions</th>
                    </tr>
                  </thead>
                    ...
                    {filteredDispatches.length === 0 ? (
                      ...
                      filteredDispatches.map((disp) => (
                        <tr key={disp.id} className="hover:bg-slate-50 dark:hover:bg-[#141414] transition-colors">
                          ...
                          <td className="py-4 px-3 xl:px-4 font-mono text-center font-bold text-slate-900 dark:text-[#ededed] whitespace-nowrap">
                            {disp.requestedBeds || 1}
                            {disp.approvedBeds !== undefined && disp.approvedBeds !== null && disp.approvedBeds < disp.requestedBeds && (
                              <span className="text-amber-600 dark:text-amber-400 font-normal text-[11px] block">
                                ({disp.approvedBeds} appr)
                              </span>
                            )}
                          </td>
                          <td className="py-4 px-3 xl:px-4 text-slate-800 dark:text-[#a1a1a1] font-medium min-w-[200px] xl:min-w-[280px] leading-relaxed break-words">
                            <div>{disp.patientCondition}</div>
                            {disp.reviewReason && (
                              <div className="mt-1 text-[11px] text-amber-800 dark:text-amber-400 font-mono">
                                ⚠️ {disp.reviewReason}
                              </div>
                            )}
                          </td>
                          ...
                          <td className="py-4 px-3 xl:px-4 font-mono text-xs text-slate-500 dark:text-[#737373] whitespace-nowrap">
                            <div>{formatDate(disp.createdAt)}</div>
                          ...
                          <td className="py-4 px-3 xl:px-4 text-center whitespace-nowrap min-w-[190px] xl:min-w-[230px]">
                            {disp.reviewRequired ? (
                              <button
                                onClick={() => setReviewDispatch(disp)}
                                className="inline-flex items-center justify-center px-4 py-1.5 text-xs font-mono font-bold uppercase tracking-wider text-white bg-slate-900 hover:bg-slate-800 dark:bg-[#1f1f1f] dark:hover:bg-[#282828] border border-slate-700 dark:border-[#3a3a3a] rounded-sm transition-colors cursor-pointer whitespace-nowrap shadow-xs"
                              >
                                REVIEW UPDATE
                              </button>
                            ) : disp.status === "PENDING" ? (
                              <div className="flex items-center justify-center gap-2 flex-wrap">
                                <button
                                  onClick={() => handleUpdateStatus(disp.id, "ACCEPTED")}
                                  disabled={updatingId === disp.id}
                                  className="px-3.5 py-1.5 text-xs font-semibold uppercase tracking-wider text-white bg-emerald-700 hover:bg-emerald-800 dark:bg-emerald-600 dark:hover:bg-emerald-700 rounded-sm transition-colors disabled:opacity-50 cursor-pointer whitespace-nowrap"
                                >
                                  Accept
                                </button>
                                <button
                                  onClick={() => handleUpdateStatus(disp.id, "REJECTED")}
                                  disabled={updatingId === disp.id}
                                  className="px-3.5 py-1.5 text-xs font-semibold uppercase tracking-wider text-white bg-red-700 hover:bg-red-800 dark:bg-red-600 dark:hover:bg-red-700 rounded-sm transition-colors disabled:opacity-50 cursor-pointer whitespace-nowrap"
                                >
                                  Reject
                                </button>
                              </div>
                              ...
                              <Link
                                href={`/dispatch-requests/${disp.id}`}
                                className="inline-flex items-center gap-1 text-xs font-mono font-semibold text-blue-700 dark:text-blue-400 hover:underline"
                              >
                                View Details →
                              </Link>
                            )}
